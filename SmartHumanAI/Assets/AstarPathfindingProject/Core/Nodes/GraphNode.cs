...
namespace Pathfinding
{
    ...
    public class SpaceSyntaxData
    {
        ...
        public static float FP1_max = 0;
        public static float FP2_max = 0;
        public static float FP3_max = 0;
        public static float FP4_max = 0;
        public static float FP5_max = 0;
        public static float FP6_max = 0;
        public static float FP7_max = 0;
        public static float FP8_max = 0;
    }
    ...
    public abstract class GraphNode
    {
        ...
        public float spreadingPenalty = 0;

        public SpaceSyntaxData SSxD = null;
        public int currentUtilization = 0;
        public float currentUtilizationSmooth = 0;
        public List<int> utilizations = new List<int>();
        public List<float> utilizations9Sq = new List<float>();
        public List<float> speeds = new List<float>();
        public float util_highest = float.MinValue;
        public float util_total = float.MinValue;
        public float util_average = float.MinValue;
        public float speedAverage = float.MinValue;
        public List<float> currentSpeeds = new List<float>();

        public void RecalculatePenalty(bool useSpaceSyntax, float fp1, float fp2, float fp3, float fp4, float fp5, float fp6, float fp7, float fp8)
        {
            if (!useSpaceSyntax || SSxD == null)
                penalty = 0;
            else
            {
                penalty = 0;
                penalty += (uint)Mathf.Abs(fp1 * SSxD.FP1 - (fp1 < 0 ? fp1 * SpaceSyntaxData.FP1_max : 0));
                penalty += (uint)Mathf.Abs(fp2 * SSxD.FP2 - (fp2 < 0 ? fp2 * SpaceSyntaxData.FP2_max : 0));
                penalty += (uint)Mathf.Abs(fp3 * SSxD.FP3 - (fp3 < 0 ? fp3 * SpaceSyntaxData.FP3_max : 0));
                penalty += (uint)Mathf.Abs(fp4 * SSxD.FP4 - (fp4 < 0 ? fp4 * SpaceSyntaxData.FP4_max : 0));
                penalty += (uint)Mathf.Abs(fp5 * SSxD.FP5 - (fp5 < 0 ? fp5 * SpaceSyntaxData.FP5_max : 0));
                penalty += (uint)Mathf.Abs(fp6 * SSxD.FP6 - (fp6 < 0 ? fp6 * SpaceSyntaxData.FP6_max : 0));
                penalty += (uint)Mathf.Abs(fp7 * SSxD.FP7 - (fp7 < 0 ? fp7 * SpaceSyntaxData.FP7_max : 0));
                penalty += (uint)Mathf.Abs(fp8 * SSxD.FP8 - (fp8 < 0 ? fp8 * SpaceSyntaxData.FP8_max : 0));
            }
        }
        public float GetAverageSpeed()
        {
            ...
            speedAverage = totalSpeed / utilCount;

            return speedAverage;
        }
        public float GetAverageDensity()
        {
            return (float)utilizations.Average() * 4f;
        }
        public float GetMaxDensity()
        {
            return utilizations9Sq.Max() * 4f;
        }
        public float GetUtilization()
        {
            return utilizations.Sum();
        }
#endif
