...
namespace Pathfinding {
	...
	public class GridNode : GridNodeBase {
        ...
        public float GetAverageSpeed_9Sq()
		{
			float sum = GetAverageSpeed();

			List<GraphNode> gns = new List<GraphNode>();
			GetConnections(gns.Add);

			foreach (var gn in gns)
				sum += gn.GetAverageSpeed();

			return sum / (gns.Count + 1);
		}
		public float GetAverageDensity_9Sq()
		{
			float sum = GetAverageDensity();

			List<GraphNode> gns = new List<GraphNode>();
			GetConnections(gns.Add);

			foreach (var gn in gns)
				sum += gn.GetAverageDensity();

			return sum / (gns.Count + 1);
		}
		public float GetMaxDensity_9Sq()
		{
			float sum = GetMaxDensity();

			List<GraphNode> gns = new List<GraphNode>();
			GetConnections(gns.Add);

			foreach (var gn in gns)
				sum += gn.GetMaxDensity();

			return sum / (gns.Count + 1);
		}
		public float GetUtilization_9Sq()
		{
			return utilizations9Sq.Sum();
		}

		readonly List<GraphNode> _gns = new List<GraphNode>();
		private int numGridCells = 0;

		public void EndOfCycle()
		{
			if (_gns.Count == 0)
			{
				// Record the neighbouring cells.
				GetConnections(_gns.Add);
				numGridCells = _gns.Count + 1;
			}

			currentUtilizationSmooth = currentUtilization;

			foreach (var gn in _gns)
				currentUtilizationSmooth += gn.currentUtilization;

			currentUtilizationSmooth /= numGridCells;

			utilizations9Sq.Add(currentUtilizationSmooth);
			utilizations.Add(currentUtilization);

			if (currentSpeeds.Count <= 0) return;
			float avrg = currentSpeeds.Average();
			speeds.Add(avrg);
		}
		public void Reset()
		{
		...
		/// <summary>
		/// Gets size of heatmap data in bytes.
		/// </summary>
		/// <returns></returns>
		public int GetSize()
		{
			return sizeof(float) * (utilizations.Count + utilizations9Sq.Count + utilizations.Count + currentSpeeds.Count);
		}

		public void ResetAll()
		{
			currentUtilization = 0;
			currentUtilizationSmooth = 0;
			utilizations.Clear();
			utilizations.TrimExcess();
			utilizations9Sq.Clear();
			utilizations9Sq.TrimExcess();
			speeds.Clear();
			speeds.TrimExcess();
			util_highest = float.MinValue;
			util_total = float.MinValue;
			util_average = float.MinValue;
			speedAverage = float.MinValue;
			currentSpeeds.Clear();
			currentSpeeds.TrimExcess();
		}
